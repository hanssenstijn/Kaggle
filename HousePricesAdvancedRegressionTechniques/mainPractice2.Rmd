...
```{r}
# set test sale price variable and give them all na
...
```{r}
numericVars <-  #index vector numeric variables
numericVarNames <-  #saving names vector for use later on
cat('There are', length(numericVars), 'numeric variables')

all_numVar <-  # save new dataset with the numericVars
cor_numVar <- cor(, use="pairwise.complete.obs") #correlations of all numeric variables
...
```{r}
ggplot(data=all[!is.na(all$SalePrice),], aes())+
...
```{r}
ggplot(all[!is.na(all$LotFrontage),], aes() +
        geom_bar(stat='summary', fun.y = "median", fill='blue') +
